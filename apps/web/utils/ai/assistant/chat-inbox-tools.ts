...
export const replyEmailTool = ({
  ...
  tool({
    description:
      "Prepare a reply to an existing email by message ID, either immediate or scheduled for later via sendAt. This does NOT send immediately — it returns a confirmation payload for the user to approve. Nothing is queued at this point, so if the user then asks to cancel or discard it, do not look for a scheduled email: say it was never sent and will not send unless they approve the card. Scheduled replies are threaded onto the original conversation. Do not recreate replies with sendEmail.",
