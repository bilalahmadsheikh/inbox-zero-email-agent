...
function getEmailCapabilitiesPolicy({
  ...
  const enabledEmailSendingLines = [
    ...
    "- Only sendEmail supports scheduling (sendAt). Replies and forwards cannot be scheduled: to schedule a follow-up to someone, use sendEmail addressed to them with a fitting subject and say it will not thread onto the conversation.",
    "- To automatically cancel or release a scheduled email when that person writes back, create a rule (createRule) with the CANCEL_SCHEDULED or SEND_SCHEDULED action and a condition matching email from that person. Never claim this is impossible.",
