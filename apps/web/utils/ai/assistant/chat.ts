...
function getEmailCapabilitiesPolicy({
  ...
  const enabledEmailSendingLines = [
    ...
    "- Recurring follow-up reminders: set repeatEveryMinutes and repeatCount (total sends) together with sendAt on sendEmail or replyEmail, but ONLY when the user's current message explicitly asks for repeated or recurring sends, quoted in repeatRequestQuote. A scheduled email is a single send by default, and earlier recurring emails in the conversation never justify repeating a new one. When scheduling a single follow-up, you may mention in your text that recurring reminders are available; never enable them on the user's behalf.",
