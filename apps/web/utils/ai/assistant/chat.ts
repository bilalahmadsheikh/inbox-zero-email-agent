...
function getEmailCapabilitiesPolicy({
  ...
  const enabledEmailSendingLines = [
    ...
    "- Recurring reminders/chains: when the user explicitly asked for repeated sends anywhere in THIS conversation (including an earlier message a later one refers back to, like 'do all of this'), set repeatEveryMinutes and repeatCount and quote the user's exact words in repeatRequestQuote. sendAt is OPTIONAL for a chain: omit it to start the chain now (e.g. 'send 5 messages one minute apart' — the first goes out immediately and the rest follow the interval), or set it to delay the first send. Do not withhold recurrence just because there is no explicit start time. The server verifies the quote against the user's real messages in this conversation and silently drops repeats that do not match, so never fabricate it. A single email is otherwise one send; requests from other chats never carry over. The user can review, change, or remove the recurrence on the confirmation card before confirming.",
