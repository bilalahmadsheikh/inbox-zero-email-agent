...
export const changelog: ChangelogEntry[] = [
  {
    ...
    notes: [
      ...
      'Compose can now write the email for you. A box above the editor takes an instruction — "ask Sarah for the Q3 numbers", "thank them and suggest Thursday" — and writes a draft in your writing style, matching the tone to whether the recipient looks like a personal contact or a business one. It adds to whatever you have already typed rather than replacing it, so you can write an opening line yourself and have the rest filled in. Replies already had this in the thread view; new emails now have it too.',
      "The compose window no longer closes when you click outside it. A half-written email is easy to lose and impossible to get back, so it now closes only on the cross or Escape.",
      'Choosing a custom send time now opens a calendar straight away, and stays open. It used to show an empty dd/mm/yyyy field you had to click a small icon to open, in a box that vanished the moment you moved the mouse off "Custom time".',
      "It writes from your instruction and your saved writing style only, and is never shown your past emails. The assistant uses those for tone, which is how a line of unrelated mail once ended up inside a new message; this route cannot make that mistake because it never receives them.",
      "Reverted an unintended change to how mail is filed. When the send-only address check was widened for choosing recipients, the narrower check that decides whether an email counts as a conversation quietly started ignoring capitalisation too, so an address written NoReply@ was newly treated as automated. Filing now behaves exactly as it did before; only recipient selection ignores capitalisation, which is where that was intended.",
      "Fixed every email account after your first starting up unconfigured. Adding a second mailbox took you straight into the app with no labels, no rules and no Reply Zero, and nothing said so — you only found out when the assistant did nothing. The cause: whether setup had been done was remembered once per person per browser, while the labels and rules it creates belong to one mailbox. Adding a new account therefore looked like it had already been set up. Setup is now decided by whether that particular mailbox has any rules, so a new account gets taken to label setup while accounts you have already configured are left alone.",
    ],
  },
  {
    version: "4.9",
    ...
    notes: [
      ...
      "Fixed wording from your old emails ending up inside a new one. Before writing to someone, the assistant looks at a few emails you have sent before so it can match your tone. Those samples were only ever meant to guide how the message reads, but a line from one could be copied into the message itself — a request to introduce yourself once produced an email whose entire body was a stray line of Gmail boilerplate. Since the recipient can be someone unconnected to the sampled mail, this is now blocked at both ends: the samples are marked as tone reference only, and the message body must be written from what you asked for.",
      "The assistant no longer offers to discard an email awaiting your confirmation. It cannot remove that card, and saying otherwise only led somewhere that could not work. It now says the email will never send unless you approve it, so the card is safe to ignore.",
    ],
  },
  {
    version: "4.8",
