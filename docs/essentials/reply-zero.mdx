...
Reply Zero labels every email that needs a reply as `To Reply`, and every email where you're waiting for a reply as `Awaiting Reply`. These labels appear in your regular email client (Gmail or Outlook). Gmail users can also use the Reply Zero view in The Inbox Intern to see only these emails.
...
Go to the [Reply Zero](/reply-zero) tab in the left sidebar to enable it.
...
- **Mark as Done** â€” Click `Mark Done` to move a conversation to the `Done` tab
- **Filter by Age** â€” Filter for emails waiting more than a week to focus on overdue threads
