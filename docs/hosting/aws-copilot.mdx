...
description: 'Deploy The Inbox Intern to AWS using AWS Copilot and ECS Fargate'
---

Deploy The Inbox Intern to AWS using AWS Copilot. The deployment uses Amazon ECS on Fargate.
...
- The Inbox Intern repository cloned locally (run all commands from the repo root)
...
```
Google Pub/Sub â†’ API Gateway (public) â†’ VPC Link â†’ Internal ALB â†’ ECS
                      â†‘
               JWT validation
               (Google OIDC)
```
