...
description: 'Reference for self-hosting environment variables used in The Inbox Intern'
---

Reference for environment variables relevant to self-hosting The Inbox Intern. Hosted-only billing, analytics, and internal operations variables are intentionally omitted unless they affect common self-hosted deployments.
...
| `DATABASE_URL_UNPOOLED` | No | Alternative unpooled PostgreSQL URL used by the app runtime in preview-style environments | â€” |
| `NEXT_PUBLIC_BASE_URL` | Yes | Public URL where app is hosted (e.g., `https://yourdomain.com`) | â€” |
| `INTERNAL_API_KEY` | Yes | Secret key for internal API calls. Generate with `openssl rand -hex 32` | â€” |
| `AUTH_SECRET` | Yes | better-auth secret. Generate with `openssl rand -hex 32` | â€” |
| `NODE_ENV` | No | Environment mode | `development` |
| **Encryption** ||||
| `EMAIL_ENCRYPT_SECRET` | Yes | Secret for encrypting OAuth tokens. Generate with `openssl rand -hex 32` | â€” |
| `EMAIL_ENCRYPT_SALT` | Yes | Salt for encrypting OAuth tokens. Generate with `openssl rand -hex 16` | â€” |
| **Google OAuth** ||||
| `GOOGLE_CLIENT_ID` | Yes | OAuth client ID from Google Cloud Console | â€” |
| `GOOGLE_CLIENT_SECRET` | Yes | OAuth client secret from Google Cloud Console | â€” |
| **Microsoft OAuth** ||||
| `MICROSOFT_CLIENT_ID` | No | OAuth client ID from Azure Portal | â€” |
| `MICROSOFT_CLIENT_SECRET` | No | OAuth client secret from Azure Portal | â€” |
...
| `MICROSOFT_WEBHOOK_CLIENT_STATE` | No | Secret for Microsoft webhook verification. Generate with `openssl rand -hex 32` | â€” |
| **Slack** ||||
| `SLACK_CLIENT_ID` | No | Slack OAuth client ID | â€” |
| `SLACK_CLIENT_SECRET` | No | Slack OAuth client secret | â€” |
| `SLACK_SIGNING_SECRET` | No | Slack signing secret used to verify requests | â€” |
| `NEXT_PUBLIC_SLACK_BOT_NAME` | No | Bot display name shown in the app | `The Inbox Intern` |
| **Messaging Adapters** ||||
| `TEAMS_BOT_APP_ID` | No | Microsoft Teams bot app ID | â€” |
| `TEAMS_BOT_APP_PASSWORD` | No | Microsoft Teams bot app password/secret | â€” |
| `TEAMS_BOT_APP_TENANT_ID` | No | Tenant ID, required when Microsoft Teams integration is enabled | â€” |
| `TELEGRAM_BOT_TOKEN` | No | Telegram bot token from BotFather | â€” |
| `TELEGRAM_BOT_SECRET_TOKEN` | No | Optional Telegram webhook secret token (sent in `x-telegram-bot-api-secret-token`) | â€” |
| **Google PubSub** ||||
| `GOOGLE_PUBSUB_TOPIC_NAME` | Yes | Full topic name (e.g., `projects/my-project/topics/gmail`) | â€” |
| `GOOGLE_PUBSUB_VERIFICATION_TOKEN` | Yes* | Token for webhook verification | â€” |
| **Redis** ||||
| `UPSTASH_REDIS_URL` | No* | Upstash Redis URL or any Upstash-compatible HTTP Redis endpoint (*required if not using Docker Compose with local Redis) | â€” |
| `UPSTASH_REDIS_TOKEN` | No* | Upstash Redis token or serverless-redis-http token (*required if not using Docker Compose) | â€” |
| `REDIS_URL` | No | Redis URL for subscriptions and the optional BullMQ worker | â€” |
| **Image Proxy (Optional)** ||||
| `NEXT_PUBLIC_IMAGE_PROXY_BASE_URL` | No | Base URL for the optional remote-image proxy. Example: `https://img.example.com/proxy` | â€” |
...
| `IMAGE_PROXY_SIGNING_SECRET` | No | Shared HMAC secret used to sign proxy URLs for the bundled Cloudflare Worker or a compatible proxy. Proxy validators may use a comma-separated list, but each signer should still be configured with a single secret. | â€” |
| **LLM Provider Selection** ||||
| `DEFAULT_LLMS` | Yes | Ordered default model list in `provider:model,provider:model` format. First valid entry is primary; later entries are fallbacks. | â€” |
...
| `DEFAULT_OPENROUTER_PROVIDERS` | No | Comma-separated list of OpenRouter providers | â€” |
| `ECONOMY_OPENROUTER_PROVIDERS` | No | OpenRouter providers for economy model | â€” |
| `CHAT_OPENROUTER_PROVIDERS` | No | OpenRouter providers for chat | â€” |
| `DEFAULT_LLM_PROVIDER` | Deprecated | Legacy primary LLM provider. Converted into `DEFAULT_LLMS` at startup. | â€” |
| `DEFAULT_LLM_MODEL` | Deprecated | Legacy default model. Converted into `DEFAULT_LLMS` at startup. | Provider default |
| `DEFAULT_LLM_FALLBACKS` | Deprecated | Legacy default fallback chain. Converted into `DEFAULT_LLMS` at startup. | â€” |
| `ECONOMY_LLM_PROVIDER` | Deprecated | Legacy economy provider. Converted into `ECONOMY_LLMS` at startup. | â€” |
| `ECONOMY_LLM_MODEL` | Deprecated | Legacy economy model. Converted into `ECONOMY_LLMS` at startup. | â€” |
| `ECONOMY_LLM_FALLBACKS` | Deprecated | Legacy economy fallback chain. Converted into `ECONOMY_LLMS` at startup. | â€” |
| `CHAT_LLM_PROVIDER` | Deprecated | Legacy chat provider. Converted into `CHAT_LLMS` at startup. | â€” |
| `CHAT_LLM_MODEL` | Deprecated | Legacy chat model. Converted into `CHAT_LLMS` at startup. | â€” |
| `CHAT_LLM_FALLBACKS` | Deprecated | Legacy chat fallback chain. Converted into `CHAT_LLMS` at startup. | â€” |
| `NANO_LLM_PROVIDER` | Deprecated | Legacy nano provider. Converted into `NANO_LLMS` at startup. | â€” |
| `NANO_LLM_MODEL` | Deprecated | Legacy nano model. Converted into `NANO_LLMS` at startup. | â€” |
| `DRAFT_LLM_PROVIDER` | Deprecated | Legacy draft provider. Converted into `DRAFT_LLMS` at startup. | â€” |
| `DRAFT_LLM_MODEL` | Deprecated | Legacy draft model. Converted into `DRAFT_LLMS` at startup. | â€” |
| **LLM Provider Credentials** ||||
| `LLM_API_KEY` | No | Shared fallback API key for simple single-provider setups; use provider-specific keys when mixing providers. | â€” |
| `ANTHROPIC_API_KEY` | No | Anthropic API key | â€” |
| `OPENAI_API_KEY` | No | OpenAI API key | â€” |
...
| `GOOGLE_API_KEY` | No | Google Gemini API key | â€” |
...
| `GROQ_API_KEY` | No | Groq API key | â€” |
| `OPENROUTER_API_KEY` | No | OpenRouter API key | â€” |
| `AI_GATEWAY_API_KEY` | No | AI Gateway API key | â€” |
| `PERPLEXITY_API_KEY` | No | Perplexity API key for guest research for meeting briefs | â€” |
| **Azure OpenAI** ||||
| `AZURE_API_KEY` | No | Azure OpenAI API key (required when `azure` is used and `LLM_API_KEY` is not set) | â€” |
| `AZURE_RESOURCE_NAME` | No | Azure OpenAI resource name (required when `azure` is used as a default or fallback provider) | â€” |
| `AZURE_API_VERSION` | No | Azure OpenAI API version override | â€” |
| **Google Vertex** ||||
| `GOOGLE_VERTEX_PROJECT` | No | Google Cloud project ID for Vertex AI (required when `vertex` is used as a default or fallback provider) | â€” |
| `GOOGLE_VERTEX_LOCATION` | No | Vertex AI location | `us-central1` |
| `GOOGLE_VERTEX_CLIENT_EMAIL` | No | Service account client email for Vertex auth (when not using ADC file) | â€” |
| `GOOGLE_VERTEX_PRIVATE_KEY` | No | Service account private key for Vertex auth (supports `\n` escaped newlines) | â€” |
| `GOOGLE_APPLICATION_CREDENTIALS` | No | Path to a Google service account JSON file for ADC/Vertex auth | â€” |
| **AWS Bedrock** ||||
| `BEDROCK_ACCESS_KEY` | No | AWS access key for Bedrock. See [AI SDK Bedrock documentation](https://ai-sdk.dev/providers/ai-sdk-providers/amazon-bedrock). | â€” |
| `BEDROCK_SECRET_KEY` | No | AWS secret key for Bedrock | â€” |
| `BEDROCK_REGION` | No | AWS region for Bedrock | `us-west-2` |
| **Ollama (Local LLM)** ||||
| `OLLAMA_BASE_URL` | No | Ollama API endpoint (e.g., `http://localhost:11434/api`) | â€” |
| `OLLAMA_MODEL` | No | Ollama model name when configured separately from the selected LLM tier model | â€” |
...
| `OPENAI_COMPATIBLE_MODEL` | No | OpenAI-compatible model name when configured separately from the selected LLM tier model | â€” |
...
| `CODEX_CLI_PATH` | No | Optional path to the `codex` binary when using `codex-cli`. | â€” |
| **AI Content Controls** ||||
...
| `QSTASH_TOKEN` | No | QStash API token (optional; fallback runs jobs via internal API + cron) | â€” |
| `QSTASH_CURRENT_SIGNING_KEY` | No | Current signing key for webhooks | â€” |
| `QSTASH_NEXT_SIGNING_KEY` | No | Next signing key for key rotation | â€” |
| `QUEUE_BACKEND` | No | Background job transport: `qstash`, `bullmq`, or `internal` | Auto-detect (`qstash` when configured, else `internal`) |
| **Sentry** ||||
| `SENTRY_AUTH_TOKEN` | No | Auth token for source maps | â€” |
| `SENTRY_ORGANIZATION` | No | Organization slug | â€” |
| `SENTRY_PROJECT` | No | Project slug | â€” |
| `NEXT_PUBLIC_SENTRY_DSN` | No | Client-side DSN | â€” |
| **Resend** ||||
| `RESEND_API_KEY` | No | API key for transactional emails | â€” |
| `RESEND_AUDIENCE_ID` | No | Audience ID for contacts | â€” |
| `RESEND_FROM_EMAIL` | No | From email address | `The Inbox Intern <updates@devaicon.com>` |
| `NEXT_PUBLIC_IS_RESEND_CONFIGURED` | No | Client-side flag indicating if Resend is configured | â€” |
| **Other** ||||
| `API_KEY_SALT` | No* | Salt used to hash external API keys. Generate with `openssl rand -hex 32`. Required when `NEXT_PUBLIC_EXTERNAL_API_ENABLED=true`. | â€” |
| `CRON_SECRET` | No | Shared secret that authenticates calls to the scheduled-task endpoints (`/api/cron/*`, `/api/watch/all`, `/api/meeting-briefs`, `/api/follow-up-reminders`). Required if you trigger these endpoints yourself instead of using the bundled Docker Compose `cron` container. Generate with `openssl rand -hex 32`. See [Scheduled Tasks](/hosting/self-hosting#scheduled-tasks). | â€” |
| `HEALTH_API_KEY` | No | API key for health checks | â€” |
| `WEBHOOK_URL` | No | External webhook URL | â€” |
| `INTERNAL_API_URL` | No | Preferred callback base URL for QStash and server-side internal callbacks | `NEXT_PUBLIC_BASE_URL` |
| `OAUTH_PROXY_URL` | No | OAuth proxy deployment URL used when callbacks should route through a separate proxy server | â€” |
| `IS_OAUTH_PROXY_SERVER` | No | Marks this deployment as the OAuth proxy server | `false` |
| `ADDITIONAL_TRUSTED_ORIGINS` | No | Comma-separated additional trusted origins for auth/CORS, including wildcard origins such as `https://*.vercel.app` | â€” |
| **Digest Controls** ||||
...
| `ADMINS` | No | Comma-separated list of admin emails | â€” |
...
| `NEXT_PUBLIC_TABS_EXTENSION_ID` | No | Chrome extension ID used for Inbox Intern Tabs sync | Built-in extension ID |
| **White Labeling (Optional)** ||||
| `NEXT_PUBLIC_BRAND_NAME` | No | Brand name used in UI text and metadata | `The Inbox Intern` |
| `NEXT_PUBLIC_BRAND_LOGO_URL` | No | Custom logo URL or public asset path (for example `/images/brand-logo.svg`) | Built-in The Inbox Intern logo |
| `NEXT_PUBLIC_BRAND_ICON_URL` | No | Custom app icon URL or public asset path | `/icon.png` |
| `NEXT_PUBLIC_SUPPORT_EMAIL` | No | Contact email shown in support links and error messages | `support@devaicon.com` |
| **Debugging** ||||
| `DISABLE_LOG_ZOD_ERRORS` | No | Disable logging Zod validation errors | â€” |
| `ENABLE_DEBUG_LOGS` | No | Enable debug logging | `false` |
| `NEXT_PUBLIC_LOG_SCOPES` | No | Comma-separated log scopes | â€” |
