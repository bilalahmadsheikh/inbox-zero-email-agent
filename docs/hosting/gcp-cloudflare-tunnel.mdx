...
description: 'Deploy The Inbox Intern on Google Cloud Compute Engine with Cloudflare Tunnel for HTTPS â€” no reverse proxy or SSL certificates required'
---

This guide covers deploying The Inbox Intern on a GCP Compute Engine VM using Cloudflare Tunnel for secure HTTPS access. Cloudflare Tunnel eliminates the need for a reverse proxy (Nginx/Caddy) and SSL certificate management â€” the tunnel connects outbound from your VM to Cloudflare's edge, so no inbound firewall ports need to be opened.

**Why Cloudflare Tunnel?**
- HTTPS with no Nginx, Caddy, or Let's Encrypt setup
- No public inbound ports required â€” the VM only makes outbound connections
...
4. **Machine configuration:**
   - Minimum: `e2-medium` (2 vCPU, 4 GB RAM) â€” add swap if using this size
   - Recommended: `e2-standard-2` (2 vCPU, 8 GB RAM)
5. **Boot disk:**
   - OS: **Debian 12** or **Ubuntu 24.04 LTS**
   - Size: **20 GB** minimum (Docker images + logs)
6. **Firewall:** leave HTTP and HTTPS checkboxes **unchecked** â€” Cloudflare Tunnel handles all ingress
7. Click **Create**

### Optional: Reserve a static external IP

GCP external IPs change if the VM is stopped. If you need SSH access from a fixed address, reserve a static IP under **VPC network â†’ IP addresses** and attach it to the VM. Cloudflare Tunnel does not require a static IP.
...
1. Go to **Zero Trust â†’ Networks â†’ Tunnels â†’ Add a tunnel**
2. Choose **Cloudflared** as the connector type
3. Name the tunnel (e.g. `inbox-zero`)
4. Follow the install instructions for **Debian** â€” copy the `cloudflared` install command shown in the dashboard and run it on your VM:

```bash
# Example â€” copy the exact command from the Cloudflare dashboard
...
Startup takes 30â€“60 seconds. Once the web container logs show the Next.js server is ready, visit your Cloudflare URL.

## 6. Update Google OAuth Redirect URIs

In [Google Cloud Console â†’ APIs & Services â†’ Credentials](https://console.cloud.google.com/apis/credentials), open your OAuth client and add:
...
- The app container may still be starting up â€” wait 60 seconds and refresh
