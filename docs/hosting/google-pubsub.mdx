...
PubSub enables real-time email notifications so The Inbox Intern is notified immediately when new emails arrive.
...
2. Go to the **Permissions** tab (you may see it labeled "Info Panel" on the right side â€” look for the "Permissions" section).
...
- `GOOGLE_PUBSUB_TOPIC_NAME` â€” the full topic name (e.g., `projects/your-project-id/topics/inbox-zero-emails`)
- `GOOGLE_PUBSUB_VERIFICATION_TOKEN` â€” the value of `TOKEN` you used in the webhook URL above
