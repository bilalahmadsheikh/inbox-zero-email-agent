...
The Inbox Intern can proxy remote email images and CSS-loaded assets so senders do not see the end user's IP address when an email is rendered.

This is optional, but recommended if you host The Inbox Intern for other people.
...
### 5. Configure the The Inbox Intern App
...
If you set `NEXT_PUBLIC_IMAGE_PROXY_BASE_URL` without `IMAGE_PROXY_SIGNING_SECRET`, The Inbox Intern emits unsigned `?u=` proxy URLs.
...
If you are self-hosting and want the simplest setup, The Inbox Intern can proxy remote images through its own Next.js app route instead of a separate service.
...
In this mode, The Inbox Intern signs image proxy URLs and routes them to `${NEXT_PUBLIC_BASE_URL}/api/image-proxy`.
