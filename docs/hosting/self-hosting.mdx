...
The easiest way to get started is with the The Inbox Intern CLI. You can either use it standalone or from within the cloned repo.
...
The Inbox Intern supports multiple background job backends for self-hosted deployments:
...
|---------|---------------|-------------|
| **Email digest** | âœ… Works (sync, no retries) | âœ… Full support |
| **Delayed/scheduled email actions** | âœ… Works via cron fallback | âœ… Full support |
| **AI categorization of senders*** | âœ… Works (sync) | âœ… Works (async with retries) |
| **Bulk inbox cleaning*** | âœ… Works (sync, no throttling) | âœ… Full support |
...
New users will automatically join the organization when they sign in. This requires exactly one organization to exist â€” create one first via the app before enabling this.
