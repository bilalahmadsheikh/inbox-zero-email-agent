...
description: 'Solutions for common issues when running The Inbox Intern'
---
...
- Check that your API key has billing credits. A ChatGPT Plus or Claude Pro subscription does **not** include API access â€” you need a separate API key with credits from the provider's developer platform.
- You can change your AI provider on the Settings page in the app.

**Can't create rules using AI prompt:**
- Same cause â€” the AI provider must be correctly configured with a valid, funded API key.
- Check logs (`inbox-zero logs -f`) for specific error messages from the AI provider.

**Emails not processing automatically (Pub/Sub):**
- Verify your Pub/Sub topic and subscription are configured correctly â€” see the [Pub/Sub setup guide](/hosting/google-pubsub).
