...
description: "Set up the Teams bot for The Inbox Intern assistant chat."
---

# Microsoft Teams Integration Setup

This guide is for self-hosted deployments that want to enable Teams chat with the The Inbox Intern assistant.

## What Teams currently supports

- AI assistant chat in **direct messages** with the Inbox Intern bot
...
2. Copy **Microsoft App ID** â€” this is your `TEAMS_BOT_APP_ID`
...
6. Copy the **Value** immediately (it's only shown once) â€” this is your `TEAMS_BOT_APP_PASSWORD`
7. Go to **Overview** and copy **Directory (tenant) ID** â€” this is your `TEAMS_BOT_APP_TENANT_ID`
...
```json
{
  ...
  "name": {
    "short": "The Inbox Intern",
    "full": "The Inbox Intern Assistant"
  },
  "description": {
    "short": "AI email assistant",
    "full": "Chat with your The Inbox Intern AI assistant directly from Teams."
  },
  "icons": {
    "outline": "outline.png",
    "color": "color.png"
...
You'll also need two icon files: a 32Ã—32 `outline.png` and a 192Ã—192 `color.png`. Zip those three files together.

**To install for testing (sideloading):**

1. In Teams, click **Apps** in the sidebar
2. Click **Manage your apps** â†’ **Upload an app**
...
## 7. Connect a user account from The Inbox Intern

Each The Inbox Intern email account links to a Teams user via a connect code:

1. In The Inbox Intern, go to **Settings** â†’ **Connected Apps**
2. Click **Connect Teams**
3. Copy the generated command: `/connect <code>`
4. Open a DM with the Inbox Intern bot in Teams
5. Send the command
...
If linking fails, generate a new code â€” codes are one-time and expire after 10 minutes.
...
```text
Use Azure CLI to set up the Microsoft Teams integration for The Inbox Intern. Create a single-tenant Entra app registration with a client secret, create an Azure Bot resource on the F0 tier, point it at https://<your-domain>/api/teams/events, enable the Teams channel, and then tell me exactly which environment variables and manual Azure/Teams UI steps are left for me to complete.
```
